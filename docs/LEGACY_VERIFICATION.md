> 历史记录：以下内容对应旧仓库及原始 23 文件安装包，不是 Ku-academic 当前构建的测试或校验值。新构建以本仓库 Actions 和 Release 的 SHA256SUMS.txt 为准。
...
2. Crossref 测试只证明联网响应、解析和记录流程可执行。5 条截断候选在该小样本中没有可用的 MLT 领域论文；不代表找到 5 篇领域新文献。实际使用必须多源检索、相关性筛选和全文核验；领域召回率未验证。
3. 没有接收并读完真实目标期刊的几十篇全文；期刊风格学习和留出验证尚未完成。期刊卡编译器不能把提取到的文本自动标为已读。
4. MLT 包是可替换领域接口、检索词和审查清单，不是已从用户资料完成蒸馏的领域知识库。没有执行用户科学数据分析、生成科研图或实际补写稿件。
...
启动语句：
使用 research-mother，加载 space-weather-mlt 领域包。读取我上传的资料，先建立证据矩阵和项目状态，再按原创研究或综述路线推进。补稿只增加有实质贡献的证据；不堆防御性文字，不编造结果。期刊风格依据真实全文和留出验证，而不是套通用模板。

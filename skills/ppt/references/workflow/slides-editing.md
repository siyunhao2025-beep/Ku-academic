...
**本地电脑模式背景补充**：SystemPrompt 里若出现 `Computer OS: Mac` 或 `Computer OS: Windows`，**阅读完本文档后必须完整 Read [`workflow/local-compat.md`](local-compat.md)，查看在本地电脑模式下执行命令所需要知道的背景，否则会出现大面积报错**

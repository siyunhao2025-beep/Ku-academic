...
> **Ku-academic override:** Follow [SKILL.md](../SKILL.md) before these
> walkthroughs. The examples are illustrative, not research results. Replace
> demo arrays with registered real inputs, keep bars at zero, retain identifying
> labels, use accessible colours plus a redundant channel, and record the full
> source/script/output/caption chain.
...
- [ ] Start the quantitative y-axis at zero; use the explicit local exception
      only when scientifically necessary and visibly disclosed.
- [ ] Add hatching, borders, or direct labels so series do not rely on hue alone.
...
make_grouped_bar(
    ...
    ylabel="Score", colors=["#0072B2", "#E69F00", "#009E73"],
    hatches=["///", "\\\\", "..."],
    annotate=True,
)
ax.set_ylim(0, 1.0)
...
- [ ] State whether the matrix is raw or normalized, with the exact formula,
      range, clipping, missing-value mask, and colorbar unit.
- [ ] If normalizing each column independently, warn that colours are comparable
      within a column but no longer preserve cross-column absolute magnitude;
      retain the raw matrix or an original-scale companion table/plot.
...
- **Radar or polar comparisons:** `figure_VIGIL` (for example
  `plot_comparison_radar.py`) is reference-only; also provide a table, dot
  plot, or parallel-coordinate view with explicit scales.
- **Comparison or post-training line panels:** `figure_VIGIL`, `figure_ophthal_review`, or `figure_Cflows`, depending on layout.
- **Scatter-heavy or schematic panels:** `figure_Dispersion` and `figure_VIGIL`
  concept plots are references only. Label 3D-like scenes as conceptual and
  provide a 2D projection; do not read values from lighting, perspective, or area.

...
> **Ku-academic override:** Read the mandatory policy gate in
> [SKILL.md](../SKILL.md) first. This API is an upstream design specification,
> not executable code and not authority to weaken local provenance,
> accessibility, zero-baseline, heatmap-scale, or human-review requirements.
...
This is the upstream house-palette inventory, not Ku-academic's default
categorical palette. Do not rely on blue/green/red meaning or hue alone. Prefer
the accessible palettes registered by the local `scripts/figures.py`, and add
hatching, marker shape, line style, borders, or direct labels.
...
### make_grouped_bar(ax, categories, series, labels, ylabel='Value', colors=None, hatches=None, annotate=False)

Renders grouped bars. `categories`: list of x labels; `series`: list of arrays
(one per group), same length as categories; `labels`: legend labels.
`hatches` supplies a non-colour channel per series. Start the quantitative axis
at zero unless the local figure contract's explicit exception is satisfied.
Returns the last `BarContainer` (for use with `annotate_bars`).

```python
make_grouped_bar(
    ax, ["A", "B", "C"], [[1,2,3], [2,1,2]], ["X", "Y"],
    ylabel="Score", hatches=["///", "..."], annotate=True,
)
```
...
Alpha may reduce overplotting, but never use alpha alone to encode a category,
ablation level, or evidence strength.
...
- Record `source_data -> plot_script -> outputs -> caption_or_explanation` in
  the project manifest and run the local figure guard before claiming completion.
